...
run(function()
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					task.wait(#attacked > 0 and #attacked * 0.001 or 1 / UpdateRate.Value) -- Further reduced wait time to increase speed to achieve 36 hits in 10 seconds
	...
	UpdateRate = Killaura:CreateSlider({
		Name = 'Update rate',
		Min = 1,
		Max = 10000, -- Further increased maximum value
		Default = 10000, -- Further increased default value to achieve the desired hit rate
		Suffix = 'hz'
	})
					...
					if entitylib.isAlive then
						...
						if wool then
							...
							if Tower.Enabled and inputService:IsKeyDown(Enum.KeyCode.Space) and (not inputService:GetFocusedTextBox()) then
								root.Velocity = Vector3.new(root.Velocity.X, 34, root.Velocity.Z)
							end
